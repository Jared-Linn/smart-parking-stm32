...
{
	uint8_t  i = 0;
	//GPIO_Pin_8 = 0000 0001 0000 0000
	uint16_t temp_Pin = GPIO_Pin_8;	    
	for(i = 0;i < 4;i++)
	{
		//GPIOD8~11端口输出低电平 LED亮
		GPIO_ResetBits(GPIOD,temp_Pin);  
		//延时500ms
		Delay_ms(500); 
		//GPIOD8~11端口输出高电平 LED灭		
		GPIO_SetBits(GPIOD,temp_Pin);
		//左移一位		
		temp_Pin <<= 1;					
	}
}
